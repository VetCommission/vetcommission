...
import { createProfessional, updateProfessional, type ProfessionalInput } from "./professionalsApi";
...
const schema = z.object({
  ...
  role: z.string().trim().min(1, "Informe a função.").max(120),
...
export function ProfessionalForm({ professional, initialMode = "view" }: { professional?: Professional; initialMode?: FormMode }) {
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: professionalQueryKeys.all });
  ...
  const { register, handleSubmit, formState: { errors } } = useForm<FormData>({
    ...
    defaultValues: {
      name: currentProfessional?.name ?? "", email: currentProfessional?.email ?? "", phone: currentProfessional?.phone ?? "",
      role: currentProfessional?.role ?? "", professionalRegistration: currentProfessional?.professionalRegistration ?? "", specialty: currentProfessional?.specialty ?? "",
    },
  });
  ...
  return (
    <Paper variant="outlined" sx={{ p: { xs: 3, md: 4 } }}>
      <Stack component="form" spacing={2} onSubmit={handleSubmit((data) => mutation.mutate(data))}>
        ...
        <TextField disabled={isReadOnly} label="Função ou cargo" error={Boolean(errors.role)} helperText={errors.role?.message} {...register("role")} />
        <TextField disabled={isReadOnly} label="Registro profissional" {...register("professionalRegistration")} />
        <TextField disabled={isReadOnly} label="Especialidade" {...register("specialty")} />
