...
import type { Professional } from "./professionalTypes";
import { createProfessional, updateProfessional } from "./professionalsApi";
import type { ProfessionalInput } from "./professionalTypes";
...
export function ProfessionalForm({
  ...
  const mutation = useMutation({
    mutationFn: (input: ProfessionalInput) =>
      currentProfessional
        ? updateProfessional(currentProfessional.id, input)
  ...
  const {
    ...
    defaultValues: {
      ...
      professionalRegistration: currentProfessional?.professionalRegistration ?? "",
  ...
  return (
    <Paper variant="outlined" sx={{ p: { xs: 3, md: 4 } }}>
      <Stack component="form" spacing={2} onSubmit={handleSubmit((data) => mutation.mutate(data))}>
        ...
        <Select
          disabled={isReadOnly || rolesQuery.isLoading}
          displayEmpty
          error={Boolean(errors.role)}
          defaultValue={currentProfessional?.role ?? ""}
          {...register("role")}
        >
        ...
        <Select
          ...
          displayEmpty
          defaultValue={currentProfessional?.specialty ?? ""}
          {...register("specialty")}
        >
        ...
        <Stack direction="row" spacing={2} sx={{ justifyContent: "flex-end" }}>
          ...
          {isReadOnly ? (
            ...
            <Button disabled={mutation.isPending} type="submit" variant="contained">
              {mode === "create" ? "Salvar cadastro" : "Salvar"}
            </Button>
          )}
        </Stack>
      </Stack>
