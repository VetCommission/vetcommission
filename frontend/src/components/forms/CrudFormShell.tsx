...
export function CrudFormShell({ title, mode, listHref, isSubmitting = false, onSubmit, onEdit, onCancel, onBeforeSubmit, submitLabel = "Salvar", editLabel = "Editar", children }: CrudFormShellProps) {
    ...
    <Paper variant="outlined" sx={{ width: "100%", p: { xs: 3, sm: 4, md: 5 } }}>
      <Stack component="form" spacing={3} onSubmit={event => { onBeforeSubmit?.(); onSubmit(event); }} noValidate>
        ...
        <Stack direction="row" spacing={2} sx={{ justifyContent: "flex-end", pt: 2, borderTop: "1px solid", borderColor: "divider" }}>
          ...
          {readOnly ? <Button type="button" variant="contained" onClick={event => { event.preventDefault(); event.stopPropagation(); onEdit(); }}>{editLabel}</Button> : <Button type="submit" variant="contained" disabled={isSubmitting}>{submitLabel}</Button>}
        </Stack>
      </Stack>
    </Paper>
  );
